...
export default class OnlyDate extends Date {
  ...
  public addDays(total: number): OnlyDate {
    return new OnlyDate(this.year, this.month, this.monthDay + total);
  }
  ...
  public getDaysBetween(compareDate: Date): number {
    ...
    const newerDate = (relation === -1) ? new OnlyDate(compareDate) : this;
    let olderDate = (relation === -1) ? this : new OnlyDate(compareDate);
  ...
  public getDaysInYear(): number {
    if (OnlyDate.isLeapYear(this.year))
      return 366;
    return 365;
  }

  public getWeeksInYear(): number {
    const firstDayOfYear = new OnlyDate(this.year, this.month, 1);
  ...
  public getNextWeekDay(weekDay: WeekDay): OnlyDate {
    const difference = weekDay - this.weekDay;
    const increment = (7 + difference) % 7;
    return new OnlyDate(this.year, this.month, this.monthDay + increment);
  }

  public getFirstMondayOfYear(): OnlyDate {
    const firstDayOfYear = new OnlyDate(this.year, this.month, 1);
  ...
  public getWeekOfYear(): number {
    ...
    if (this.isBefore(firstMondayOfYear))
      return new OnlyDate(this.year - 1).getWeeksInYear();
